...
<?php
// initialisatie
$bestandsnaam = "reserveringen_data.csv";
$aantal_velden = 6; // aantal velden per voorstelling
...
if (file_exists($bestandsnaam)){
	...
	echo "<h2>De reserveringen file bestaat!</h2>" ;	
}
else{
...
$veld = fgetcsv($fp, 1000,","); //de voorstellingen komen in het array uitslag
	...
	echo "<br>";	
	$query = "INSERT INTO `schouwburg`.`reserveringen` (`bezoekersnr`, `datum`, `tijd`, `voorstellingsnr`, 
	`zaalnr`, `aantal`) VALUES ('$veld[0]','$veld[1]','$veld[2]','$veld[3]',$veld[4],'$veld[5]')";
